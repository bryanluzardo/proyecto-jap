...
export async function initProducts() {
  if (_productsInitDone) {
    // si ya inicializamos antes, reseteamos estado necesario y re-renderizamos con el catID actual
  ...
  // Buscador debounced: delegamos al input cuando exista
  const search = document.querySelector("#buscador");
  if (search) {
    search.addEventListener("input", debounce(() => {
      buscar({ list: productosAPI, input: search.value.trim().toLowerCase() });
    }, 500));
  }
  ...
  // Carga inicial según catID actual
  const catID = localStorage.getItem("catID");
  if (!catID) {
    const divProductos = document.getElementById("productos");
    if (divProductos) divProductos.innerHTML = "<p>No se seleccionó categoría.</p>";
    return;
  }
  ...
  async function loadAndRender(catID) {
    ...
    const divProductos = document.getElementById("productos");
    const contenedor = document.getElementById("contenedorProductos") || document.getElementById("productos");
    ...
    try {
      ...
      if (divProductos) {
        ...
        h2.textContent = `Te encuentras en la categoría: ${data.catName || "Sin nombre"}`;
      ...
      if (divProductos) divProductos.innerHTML = "<p>Error cargando productos. Reintentá luego.</p>";
    }
  }
  ...
  function aplicarFiltrosYOrden() {
    ...
    if (criterio === "relevancia") lista.sort((a, b) => b.soldCount - a.soldCount);
